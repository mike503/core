...
function core_router_init() {
  if ($cached = core_cache_get('router', 'route:' . $GLOBALS['request']['path'])) {
    ...
    if (core_router_regenerate()) {
      require $GLOBALS['config']['application_root'] . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'routes.cached.php';
    }
    else {
      $routes = array();
    }
    ...
    foreach ($routes as $pattern => $file) {
      if (preg_match('|' . $pattern . '|', $GLOBALS['request']['path'], $args)) {
        if (file_exists($GLOBALS['config']['application_root'] . DIRECTORY_SEPARATOR . 'pages' . DIRECTORY_SEPARATOR . $file)) {
          $GLOBALS['request']['route'] = array(
            'file' => $file,
            'arguments' => $args,
...
function core_router_regenerate($force = FALSE) {
  $route_file = $GLOBALS['config']['application_root'] . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'routes.php';
  $cache_file = $GLOBALS['config']['application_root'] . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'routes.cached.php';
